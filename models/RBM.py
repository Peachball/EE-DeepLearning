...
import pickle
from models.DeepLearning import readMNISTData, readcv, miniBatchLearning, init_weights
from models.DeepLearning import saveParams, loadParams
from experiments.DCGAN import load_images
...
class ConvRBMLayer:
    ...
    def sample_vhv(self, v, pool=None):
        ...
        [v_act, v_sig, v_bin] = self.getVisible(h_sig)
        return v_sig, v_bin
    ...
    def getPCDUpdates(self, v, persistent, steps=1, pooling=None, sparsity=False):
        def gibbsSample(prev_sample):
            v_sig, v_bin = self.sample_vhv(prev_sample)
            return v_bin, v_sig

        [samples, sig_samples], updates = theano.scan(fn=gibbsSample,
    ...
    def getCDUpdates(self, v, cd_size, steps=1, pooling=None):
        ...
        [samples, sig_samples], updates = theano.scan(gibbsSample,
                outputs_info=[v[:cd_size], None], n_steps=steps)

        new_chain = samples[-1]
        sig_sample = sig_samples[-1]
        upd = list(updates.items())
        free_energy = T.mean(self.free_energy(v)) -\
                T.mean(self.free_energy(sig_sample))

        grad = T.grad(free_energy, self.params, consider_constant=[sig_sample])
        return upd, free_energy, grad

def getMomentumUpdates(grad, params, alpha=0.1, momentum=0.9):
    mparams = []
    updates = []
    for p, g in zip(params, grad):
        m = theano.shared(np.zeros(p.get_value().shape)
                .astype(theano.config.floatX))
        mparams.append(m)
        updates.append((p, p - m))
        updates.append((m, momentum * m + alpha * g))
    return updates
...
def load_simple_image(filename):
...
def ConvRBMTester():
    ...
    X = T.tensor4()
    rbm = ConvRBMLayer(3, 24, (3, 3))
    energy = rbm.free_energy(X)
    persistent = theano.shared(np.random.rand(2, 3, 32, 32).astype(theano.config.floatX))
    upd, error, grad = rbm.getCDUpdates(X, 2, steps=1)

    origupd = upd
    upd += [(p, p - 0.1 * g) for p, g in zip(rbm.params, grad)]
