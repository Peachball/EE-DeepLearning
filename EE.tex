...
Although the models may have differnt structures, in general all the listed
...
\begin{table}[H]
	\begin{tabular}{c|c}
		Model type: & Time model took to train 3 epochs \\\hline
		1 Layer GRU& \\
		2 Layer GRU& \\
		3 Layer GRU& \\
		1 Layer Clockwork RNN& \\
		2 Layer Clockwork RNN& \\
		3 Layer Clockwork RNN& \\
		1 Layer LSTM& \\
		2 Layer LSTM& \\
		3 Layer LSTM& \\
	\end{tabular}
	\caption{Time each tested model took to converge to error}
\end{table}

The restricted boltzmann machine and autoencoder did not decrease their
reconstruction error to a significant amount, and did not even overfit the
data when they had enough hidden units, which suggests that they were
implemented poorly or that they take a significant amount of time to train.
Thus, due to limitations in resources, they do not have benchmarks, while the
other models do.
